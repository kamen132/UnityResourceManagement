...
public class FileManager
{
    ...
    public static FileCatchData ReadStreamingAssets(string path)
    {
        if (MacroDefinition.UNITY_ANDROID() && MacroDefinition.UNITY_EDITOR() == false)
        {
            using (var www = UnityWebRequest.Get(path))
            {
                var request = www.SendWebRequest();
                FileCatchData resultData = new FileCatchData();
                while (!request.isDone) { }
                byte[] result = request.webRequest.downloadHandler.data;
                if (result.Length == 0)
                {
        ...
        FileCatchData data = ReadAllBytes(path);
        return data;
    }
